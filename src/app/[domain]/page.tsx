...
// Cette page gère à la fois la racine du sous-domaine (/) et les potentiels chemins enfants
export default async function DomainPage({ params }: { params: Promise<{ domain: string }> }) {
  const resolvedParams = await params;
  const domain = decodeURIComponent(resolvedParams.domain);

  // Chercher le site par son subdomain OU son customDomain
  ...
  if (!site) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-slate-50 text-center px-4">
        <h1 className="text-4xl font-bold text-slate-900 mb-4">404 - Site introuvable</h1>
        <p className="text-slate-600 mb-8 max-w-md">
          Ce domaine ne correspond à aucun site hébergé sur notre plateforme.
        </p>
        <a href="https://sitegenerator-production.up.railway.app" className="text-indigo-600 font-medium hover:underline">
          Créer un site avec SiteGenerator
        </a>
      </div>
    );
  }
  ...
  // Mapping standard tailwind colors
  const bgMap: Record<string, string> = {
    orange: 'bg-orange-600', indigo: 'bg-indigo-600', emerald: 'bg-emerald-600', rose: 'bg-rose-600'
  };
  const textMap: Record<string, string> = {
    orange: 'text-orange-600', indigo: 'text-indigo-600', emerald: 'text-emerald-600', rose: 'text-rose-600'
  };
  const lightBgMap: Record<string, string> = {
    orange: 'bg-orange-50', indigo: 'bg-indigo-50', emerald: 'bg-emerald-50', rose: 'bg-rose-50'
  };
  ...
  // On retourne exactement le même rendu public que dans l'aperçu
  return (
